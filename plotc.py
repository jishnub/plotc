...
def apj_1col_format(fig,scale=0.5,default_fontsize=12):
    # The scale is as you set it in your latex code 
    # 50% is a good scale to use if you want rasterized images
    # at good resolutions
    rc('text', usetex=True)
    rc('font',**{'family':'serif','serif':['Times'],'size':default_fontsize/scale})
    axes = fig.get_axes()
    for ax in axes:
        axes_label_fontsize=default_fontsize/scale*4/5
        ax.xaxis.label.set_size(axes_label_fontsize)
        ax.yaxis.label.set_size(axes_label_fontsize)
        ax.title.set_size(axes_label_fontsize)
        # Axis tick labels are somewhat smaller
        axes_ticklabel_fontsize = axes_label_fontsize*5/6
        ax.tick_params(axis="both",labelsize=axes_ticklabel_fontsize)
        # Axis offset_text is smaller
        ax.yaxis.get_offset_text().set_fontsize(axes_ticklabel_fontsize)
        ax.xaxis.get_offset_text().set_fontsize(axes_ticklabel_fontsize)
        
        # Legend fontsize
        legend = ax.get_legend()
        if legend is not None:
            legend_texts = legend.get_texts()
            legend_fontsize = axes_label_fontsize*4/5
            for text in legend_texts: text.set_fontsize(legend_fontsize)
    
    default_image_width,default_image_height = 3,2 #inches
    figure_width,figure_height = default_image_width/scale, default_image_height/scale
    fig.set_size_inches(figure_width,figure_height)
    
def apj_2col_format(fig,scale=1.0,default_fontsize=15):
    # The scale is as you set it in your latex code
    rc('text', usetex=True)
    rc('font',**{'family':'serif','serif':['Times'],'size':default_fontsize/scale})
    axes = fig.get_axes()
    ...
    for ax in axes:
        axes_label_fontsize=default_fontsize/scale
        ax.xaxis.label.set_size(axes_label_fontsize)
        ax.yaxis.label.set_size(axes_label_fontsize)
        ax.title.set_size(axes_label_fontsize)
        # Axis tick labels are somewhat smaller
        ax.tick_params(axis="both",labelsize=axes_label_fontsize*5/6)
        
    default_image_width,default_image_height = 12,8 #inches
    figure_width,figure_height = default_image_width/scale, default_image_height/scale
    fig.set_size_inches(figure_width,figure_height)
...
def colorplot(arr,**kwargs):
    ...
    hide_xticks,hide_x2ticks,hide_xticklabels,hide_x2ticklabels,
    hide_yticks,hide_y2ticks,hide_yticklabels,hide_y2ticklabels,
    locator_properties_x,locator_properties_x2,locator_properties_y,locator_properties_y2,
    polar,title,subplot_index,subplot_properties,title_properties,usetex,vmin,vmax,
    x,x2,x2tick_locator,xlabel,xlabel_properties,xr,xrpad,yrpad,x_sci,xscilimits,xtick_locator,xtickpad,
    y,y2,y2tick_locator,ylabel,ylabel_properties,yr,y_sci,yscilimits,ytick_locator,ytickpad,kwargs) = _get_common_args(kwargs)
    
    if x is None: x=_np.arange(ash[1])
    if y is None: y=_np.arange(ash[0])
    ...
    if xlabel: ax.set_xlabel(xlabel,**xlabel_properties)
    if ylabel: ax.set_ylabel(ylabel,**ylabel_properties)
...
def _get_common_args(kwargs):
    ...
    if sharey is not None: subplot_properties['sharey']=sharey
    sharex = kwargs.pop('sharex',None)
    if sharex is not None: subplot_properties['sharex']=sharex
    ...
    xlabel_properties=kwargs.pop('xlabel_properties',{})
    ylabel_properties=kwargs.pop('ylabel_properties',{})
    xylabel_properties=kwargs.pop('xylabel_properties',{})
    ...
    xlabel_properties = axes_properties.get('xlabel_properties',xlabel_properties)
    ylabel_properties = axes_properties.get('ylabel_properties',ylabel_properties)
    xylabel_properties = axes_properties.get('xylabel_properties',xylabel_properties)
    
    xlabel_properties.update(xylabel_properties)
    ylabel_properties.update(xylabel_properties)
    
    return (ax,axes_properties,centerzero,cmap,colorbar,colorbar_properties,flipx,flipy,
            hide_xticks,hide_x2ticks,hide_xticklabels,hide_x2ticklabels,
            hide_yticks,hide_y2ticks,hide_yticklabels,hide_y2ticklabels,
            locator_properties_x,locator_properties_x2,locator_properties_y,locator_properties_y2,
            polar,title,subplot_index,subplot_properties,title_properties,usetex,vmin,vmax,
            x,x2,x2tick_locator,xlabel,xlabel_properties,xr,xrpad,yrpad,x_sci,xscilimits,xtick_locator,xtickpad,
            y,y2,y2tick_locator,ylabel,ylabel_properties,yr,y_sci,yscilimits,ytick_locator,ytickpad,kwargs)
...
def quiver2D(U,V,**kwargs):
    ...
    if xlabel is not None:
        ax.set_xlabel(xlabel,**xlabel_properties)
    if ylabel is not None:
        ax.set_ylabel(ylabel,**ylabel_properties)
...
def sphericalplot(arr,**kwargs):
    ...
    zlabel_properties=kwargs.pop('zlabel_properties',{})
    xyzlabelproperties=kwargs.pop('xyzlabelproperties',{})
    
    xlabel_properties.update(xyzlabelproperties)
    ylabel_properties.update(xyzlabelproperties)
    zlabel_properties.update(xyzlabelproperties)
    ...
    #~ Actual plot
    surf=ax.plot_surface(x, y, z, facecolors=cmap(norm(arr)),**kwargs)
    ...
    if xlabel: ax.set_xlabel(xlabel,**xlabel_properties)
    if ylabel: ax.set_ylabel(ylabel,**ylabel_properties)
    if zlabel: ax.set_zlabel(zlabel,**zlabel_properties)
    ...
    return PlotCollection(ax,cb,surf)
